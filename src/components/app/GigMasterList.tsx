...
/** A gig row with its (resolved) production company name for the subtitle. */
export type GigRowData = FilteredGig & { company_name?: string | null };
...
export default function GigMasterList({ gigs }: { gigs: GigRowData[] }) {
        ...
        {visible.length === 0 ? (
          ...
          <div className="p-3 flex flex-col gap-2.5">
            {visible.map((g) => {
              ...
              return <GigRow key={g.id} gig={g} active={active} />;
            })}
          </div>
        )}
      </div>
...
function GigRow({ gig, active }: { gig: GigRowData; active: boolean }) {
  ...
  return (
    <MasterRow
      ...
      selected={active}
      ariaCurrent={active}
      showThumb={!!gig.image_url}
      image={gig.image_url}
      title={gig.title || "Untitled gig"}
      subtitle={gig.company_name || undefined}
      meta={meta || undefined}
      value={money(earned)}
      badge={<StatusPill status={status} small />}
      trailing={status === "paid" ? "Fully paid" : remaining > 0 ? `${money(remaining)} outstanding` : undefined}
    />
  );
}
